...
let
    vars = (s = DoubleRegge.s0, s1 = 4.0^2, cosθ = 0.4, ϕ = π/4, t2 = -0.45)
...
let
    vars = (s = DoubleRegge.s0, s1 = 2.32^2, cosθ = cos(π/3), ϕ = π/4, t2 = -0.2)
...
let
    vars = (s = DoubleRegge.s0, s1 = 2.32^2, cosθ = cos(2π/3), ϕ = -π/4, t2 = -0.2)
    contv = get_control_variables(vars)
