...
function maybe_vspan!(; sp = nothing, xs)
    ...
    kwargs =
        sp === nothing ? (; lab = "", α = 0.1, seriescolor = 7) :
        (; sp = sp, lab = "", α = 0.1, seriescolor = 7)
    vspan!(span; kwargs...)
end
...
function bootstrap_stdev(f, sampler; nsamples)
    ...
    stat = hcat([f(sampler()) for _ = 1:nsamples]...)
...
function rand_amp(expansion)
    sampled = Vector{NamedTuple{(:I, :ϕ),Tuple{Float64,Float64}}}(
        collect(
            NamedTuple{(:I, :ϕ)}((max(0.0, rand(pw.I)), rand(pw.ϕ))) for
            pw in expansion.PWs
        ),
    )
...
fitdata = fitdata[select_bin_indices(
    length(fitdata);
    stride = BIN_STRIDE,
    max_bins = MAX_FIT_BINS,
)]
# plot 
plotdata = filter(data) do x
    inlims(x.x, (2.4, 3.0))
end
plotdata = plotdata[select_bin_indices(
    length(plotdata);
    stride = BIN_STRIDE,
    max_bins = MAX_PLOT_BINS,
)]
...
# 
model_integral(m; pars = pfr) =
    integrate_dcosθdϕ((cosθ, ϕ) -> abs2(fixed_model_sqrtq(m, cosθ, ϕ; pars = pars)))[1]
model_integral_forward(m; pars = pfr) = integrate_dcosθdϕ(
    (cosθ, ϕ) -> abs2(fixed_model_sqrtq(m, cosθ, ϕ; pars = pars)),
    (0, 1),
)[1]
model_integral_backward(m; pars = pfr) = integrate_dcosθdϕ(
    (cosθ, ϕ) -> abs2(fixed_model_sqrtq(m, cosθ, ϕ; pars = pars)),
    (-1, 0),
)[1]
...
const Np = length(pfr)
δ(i; n = Np) = (1:n .== i)
integral_interf(m, i, j) =
    model_integral(m; pars = pfr .* δ(i) + pfr .* δ(j)) -
    model_integral(m; pars = pfr .* δ(i)) - model_integral(m; pars = pfr .* δ(j));
# 
contribution_matrix(m) =
    [(i > j ? 0 : integral_interf(m, i, j) / (i == j ? 2 : 1)) for i = 1:Np, j = 1:Np]
if !SKIP_CONTRIBUTIONS
    let
        m = sum(contribution_matrix.(fitdata.x))
        heatmap(
            [mi == 0.0 ? NaN : mi for mi in m],
            xticks = (1:Np, getproperty.(model.exchanges, :label)),
            yticks = (1:Np, getproperty.(model.exchanges, :label)),
            colorbar = false,
        )
        # 
        mn = m ./ sum(m)
        for i in CartesianIndices(m)
            ...
            annotate!([(
                ...
                i[1],
                text(
                    "$(Int(round(m[i], digits=0))) / $(Int(round(100*mn[i], digits=0)))%",
                    10,
                    :red,
                ),
            )])
        ...
        s = prod(
            "$l: $v,\n" for (l, v) in
            zip(getproperty.(model.exchanges, :label), round.(pfr, digits = 2))
        )
...
data_forward_in_bins =
    [quadgk(cosθ -> safe_dNdcosθ(cosθ, a), 0, 1)[1] for a in plotdata.amps]
data_backward_in_bins =
    [quadgk(cosθ -> safe_dNdcosθ(cosθ, a), -1, 0)[1] for a in plotdata.amps]
# 
asymmetry(f, b) = (f - b) / (f + b)
δasymmetry(f, b, δ) = 2δ * sqrt(f^2 + b^2) / (f + b)^2

asymm_data =
    [asymmetry(f, b) for (f, b) in zip(data_forward_in_bins, data_backward_in_bins)]
δasymm_data = [
    δasymmetry(f, b, δ) for (f, b, δ) in
    zip(data_forward_in_bins, data_backward_in_bins, sqrt.(sum.(x -> x^2, plot_δI) / 2))
]
#
asymm_model = [
    asymmetry(f, b) for (f, b) in zip(intensity_forward_in_bins, intensity_backward_in_bins)
]
# 
let
    plot(
...
function phiasymmplot(l)
    plot(
        xlab = L"m_{\eta\pi}\,(\textrm{GeV})",
        ylab = (l == 1 ? L"<\cos\,\phi>" : L"<\cos\,2\phi>"),
        size = (500, 350),
    )
    plot!(
        plotdata.x,
        phi_moment.(setfirstargument.(plotdata.x, intensity), l; forward = true),
        lw = 2,
        c = 2,
        lab = "forward",
    )
    plot!(
        plotdata.x,
        phi_moment.(setfirstargument.(plotdata.x, intensity), l; forward = false),
        lw = 2,
        c = 3,
        lab = "backward",
    )
    ...
    err = bootstrap_stdev(
        amps -> phi_moment_data(amps; l = l, forward = true),
        () -> rand_amps(plotdata);
        nsamples = PHI_BOOTSTRAP_SAMPLES,
    )
    maybe_scatter!(plotdata.x, vals; yerr = err, c = 2, lab = "")
    #
    vals = phi_moment_data(plotdata.amps; l = l, forward = false)
    err = bootstrap_stdev(
        amps -> phi_moment_data(amps; l = l, forward = false),
        () -> rand_amps(plotdata);
        nsamples = PHI_BOOTSTRAP_SAMPLES,
    )
...
let
    ...
    function make_plot(bin)
        ...
        #
        err = bootstrap_stdev(
            amps -> safe_dNdcosθ.(cosθv, Ref(amps[bin])),
            () -> rand_amps(fitdata);
            nsamples = COSTHETA_BOOTSTRAP_SAMPLES,
        )
        #
...
bar(
    [sum(getindex.(fit_I, i)) for (i, LM) in enumerate(LMs)],
    xticks = (1:7, tolab.(LMs)),
    xlab = "LM",
    ylab = "intensity",
    lab = "",
)
...
bar(
    [sum(getindex.(pw_intensities[6:end], i)) for (i, LM) in enumerate(LMs)],
    xticks = (1:7, tolab.(LMs)),
    xlab = "LM",
    ylab = "intensity",
    lab = "",
...
produced_files = filter(
    !isnothing,
    [
        "intensity-assymetry_$(tag)_Np=$(length(settings["exchanges"]))_alpha=$(settings["scale_α"]).pdf",
        "cos-distributions_$(tag)_Np=$(length(settings["exchanges"]))_alpha=$(settings["scale_α"]).pdf",
        SKIP_CONTRIBUTIONS ? nothing :
        "contributions_$(tag)_Np=$(length(settings["exchanges"]))_alpha=$(settings["scale_α"]).pdf",
        "odd-and-even_$(tag)_Np=$(length(settings["exchanges"]))_alpha=$(settings["scale_α"]).pdf",
        "intensity-cosphi_$(tag)_Np=$(length(settings["exchanges"]))_alpha=$(settings["scale_α"]).pdf",
        "pw-projections_$(tag)_Np=$(length(settings["exchanges"]))_alpha=$(settings["scale_α"]).pdf",
...
if MERGE_PDFS
    let
        ...
        outputfile = joinpath(
            pathtofolder,
            "combined_$(tag)_Np=$(length(settings["exchanges"]))_alpha=$(settings["scale_α"]).pdf",
        )
