...
function half_angle_substitution(L; Lmax = 6)
    ...
    poly =
        simplify(
            expr.subs(sin(θ), 2t/(1+t^2)).subs(cos(θ), (1-t^2)/(1+t^2)) * (1+t^2)^Lmax,
        )/t
    poly.subs(t^2, t)
end
...
function tanθh_coeff_wignerd(L; Nmax = 5)
    ...
    [poly.coeff(t, n) for n = 0:Nmax]
end

# ╔═╡ 79e93062-ac1c-4704-99e0-3302263301fd
cs = [tanθh_coeff_wignerd(L; Nmax = 5) for L = 1:6];
...
all_amplitudes = [amplitude_from_roots(rootsi(i))*coeff0[end] for i = 0:(2^5-1)]
...
amplitude_from_roots(conjugate_subvector(roots0, conjindices(0, length(roots0)) .!= 0))
...
[sympy.integrate(dl(L)^2*sin(θ), (θ, 0, PI)) for L = 1:6]
...
scatter([Polynomial(sum(amplitude_from_roots(rootsi(i)) .* fcs))(1.1) for i = 0:31])
