...
function DoubleReggeModel(exchanges, t2::Real, scalar_α::Real, reaction_system, pars; s2shift::Real = 0.0)
    length(exchanges) == length(pars) ||
        throw(DimensionMismatch("expected $(length(exchanges)) parameters, got $(length(pars))"))
    return DoubleReggeModel(exchanges, Float64(t2), Float64(scalar_α), reaction_system, pars, Float64(s2shift))
end

with_parameters(model::DoubleReggeModel, pars) =
    DoubleReggeModel(model.exchanges, model.t2, model.scalar_α, model.reaction_system, pars; s2shift = model.s2shift)

_model_vars(model::DoubleReggeModel, m, cosθ, ϕ) =
    (s = model.reaction_system.s0, s1 = m^2, cosθ = cosθ, ϕ = ϕ, t2 = model.t2)

function modelDR(α1oft::trajectory, α2oft::trajectory, vars, system;
    η_forward::Bool=true,  α′::Float64 = 0.9, s2shift::Float64 = 0.0)
    # 
...
modelDR(exchange::ReggeExchange, vars, reaction_system; α′::Float64 = 0.9, s2shift::Float64 = 0.0) =
    modelDR(exchange.α_top, exchange.α_bot, vars, reaction_system;
        η_forward = exchange.η_forward, α′ = α′, s2shift = s2shift)

function amplitude(model::DoubleReggeModel, m, cosθ, ϕ)
    vars = _model_vars(model, m, cosθ, ϕ)
    generator = (p * modelDR(exchange, vars, model.reaction_system;
        α′ = model.scalar_α, s2shift = model.s2shift)
