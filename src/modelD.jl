...
function build_model(exchanges, t2::Float64, scalar_α, reaction_system; s2shift=0.0)
    function model(m,cosθ,ϕ; pars)
        vars = (s = reaction_system.s0, s1 = m^2, cosθ = cosθ, ϕ = ϕ, t2 = t2)
        generator = (p*modelDR(t[1], t[2], vars, reaction_system;
            η_forward=t[3], α′=scalar_α, s2shift=s2shift)
