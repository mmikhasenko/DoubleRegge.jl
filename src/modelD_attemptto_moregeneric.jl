...
modelDR(exchs::TopBottomExchanges, vars, reaction_system, α′) =
    modelDR(exchs.α_top, exchs.α_bot, vars, reaction_system; exchs.η_forward, α′ = 0.9)
#
function modelDR(
    α1oft::trajectory,
    α2oft::trajectory,
    ...
    prefactor = -K * sf_gamma(1-α1) * sf_gamma(1-α2) * (α′*s1)^α1 * (α′*s2)^α2 / (α′*s)
    #
    # vertex funcions
    τ1 = τ2 = 1.0
    η = s/(α′*s1*s2)
...
function build_model(exchanges, t2::Float64, scalar_α, reaction_system)
    function model(m, cosθ, ϕ; pars)
        ...
        generator = (
            p*modelDR(t[1], t[2], vars, reaction_system; η_forward = t[3], α′ = scalar_α) for (p, t) in zip(pars, exchanges)
        )
