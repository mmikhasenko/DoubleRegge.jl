...
#
function sπp(vars, system)
    @unpack s, s1, cosθ, ϕ, t2 = vars
    @unpack mb², mt², mr², m1², m2² = diffraction(system)^2
    sinθ = sqrt(1-cosθ^2)
    _cosθ2 = cosθ2(vars, system)
...
q(m, system) = let reaction = diffraction(system)
    sqrt(λ(m^2, reaction.m1^2, reaction.m2^2))/m
end
