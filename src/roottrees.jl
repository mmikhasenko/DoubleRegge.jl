...
function list_of_vectors(
    ...
    tpl = NamedTuple{varnames}([
        1e3 .* SVector(
            [
                getproperty(tree_entry, Symbol(before * p * between * c)) for
                c in ['X', 'Y', 'Z', 'E']
            ]...,
...
function cosθη_of_s1t1(vars; m1, m2)
    @unpack s1, t1, t2 = vars
    return (2s1 * (t1 - m2^2 - m1^2) + (s1 - t2 + mb^2) * (s1 + m1^2 - m2^2)) /
           sqrt(λ(s1, t2, mb^2) * λ(s1, m1^2, m2^2))
end
