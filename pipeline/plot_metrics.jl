...
model_integral_forward(m; pars = pfr) =
    integrate_dcosθdϕ((cosθ, ϕ)->abs2(fixed_model_sqrtq(m, cosθ, ϕ; pars = pars)), (0, 1))[1]
model_integral_backward(m; pars = pfr) =
    integrate_dcosθdϕ((cosθ, ϕ)->abs2(fixed_model_sqrtq(m, cosθ, ϕ; pars = pars)), (-1, 0))[1]
...
let
    ...
    s = prod(
        "$l: $v,\n" for
        (l, v) in zip(getproperty.(model.exchanges, :label), round.(pfr, digits = 2))
    )
...
data_forward_in_bins =
    [quadgk(cosθ->dNdcosθ(cosθ; amps = a, LMs = LMs), 0, 1)[1] for a in plotdata.amps]
data_backward_in_bins =
    [quadgk(cosθ->dNdcosθ(cosθ; amps = a, LMs = LMs), -1, 0)[1] for a in plotdata.amps]
# 
...
δasymm_data = [
    ...
    zip(data_forward_in_bins, data_backward_in_bins, sqrt.(sum.(x->x^2, plotdata.δI)/2))
]
#
asymm_model = [
...
let
    ...
    scatter!(
        ...
        lab = "forward",
        common_options...,
        yerr = sqrt.(sum.(x->x^2, plotdata.δI)/2),
        seriescolor = 3,
    )
    scatter!(
        sp = 1,
...
phi_moment_data(amps; l, forward) = phi_moment.(
    setthirdfourtharguments.(amps, Ref(LMs), (x...)->abs2(recamp(x...))),
    l;
    forward = forward,
)
#
...
function phiasymmplot(l)
    ...
    err = take_stdiv_of_vector([
        phi_moment_data(randA(plotdata); l = l, forward = true) for _ = 1:100
    ])
    scatter!(plotdata.x, vals, yerr = err, c = 2, lab = "")
    #
    vals = phi_moment_data(plotdata.amps; l = l, forward = false)
    err = take_stdiv_of_vector([
        phi_moment_data(randA(plotdata); l = l, forward = false) for _ = 1:100
    ])
...
let
    ...
    function make_plot(bin)
        ...
        #
        stat = hcat(
            [dNdcosθ.(cosθv; amps = randA(fitdata)[bin], LMs = LMs) for _ = 1:1000]...,
        )
...
let
    ...
    for (i, (L, M)) in enumerate(LMs)
        scatter!(
            sp = i,
            plotdata.x,
            getindex.(plotdata.I, i),
            yerr = getindex.(plotdata.δI, i),
...
bar(
    [sum(getindex.(fitdata.I, i)) for (i, LM) in enumerate(LMs)],
...
let
    ...
    outputfile = joinpath(
        pathtofolder,
        "combined_$(output_tag)_Np=$(length(settings["exchanges"]))_alpha=$(settings["scale_α"]).pdf",
    )
    # inputfiles = filter(f->splitext(f)[2]==".pdf" && f!=outputfile, inputfiles)
